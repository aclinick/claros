...
timestamp, regenerate, and avoid re-recording.
...
| --- | --- | --- |
| `<file>` | Subtitle file, `.srt` or `.vtt` (required). | None |
...
To fix an overrun, shorten the line or widen the gap in the subtitle file; this
is the same edit-and-regenerate loop. This sample does not time-stretch audio to force a
