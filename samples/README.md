...
Manage voices). The `NaturalVoiceSynthesizer`-based samples (Demo, BatchSynthesis,
LowLevelPipeline) also need the stock **Microsoft Zira Desktop** SAPI voice; the
`EmbeddedSpeechSynthesizer` samples (SpeakWebPage, SpeakSubtitles) do not. The
...
| [BatchSynthesis](BatchSynthesis) | Load one `NaturalVoiceSynthesizer` and synthesize many phrases to separate WAV files; tune `SynthesisOptions`. | `dotnet run --project samples\BatchSynthesis\Claros.BatchSynthesis.csproj` |
| [SpeakWebPage](SpeakWebPage) | Flagship forced-HD `EmbeddedSpeechSynthesizer`: fetch a web page, extract its text, and narrate it to a WAV. | `dotnet run -r win-arm64 --project samples\SpeakWebPage\Claros.SpeakWebPage.csproj -- <url>` |
