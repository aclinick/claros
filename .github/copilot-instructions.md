...
- `Claros.slnx` is the solution (XML `slnx` format); it aggregates the
  library, the Demo, and the test project. `global.json` pins the SDK to the 10.x band.
- **Tests:** xUnit project at `tests\Claros.Tests`. It covers only the
...
- **Namespacing:** public API in `Claros`; helpers in
  `Claros.Internal`. Windows-only public types carry
